...
def experience_func(policynet, targetvaluenet, train_queue):
    
    
    env = gym.make("CartPole-v0")
    policy = SamplingSinglePolicy(policynet)
    exp_source = ExperienceSourceForPolicy(env, n_steps = N_STEPS, gamma=GAMMA)
    ...
    while True:
        ...
        data = get_batch(batch, targetvaluenet, n_steps=N_STEPS, gamma=GAMMA)
...
if __name__ == '__main__':
    ...
    #Setting parameters
    env = gym.make('CartPole-v0')
    policynet = PolicyNet(4, env.action_space.n)
    policynet.share_memory()

    valuenet = ValueNet(4)
    targetvaluenet = ValueNet(4)
    ...
    for _ in range(NUM_PROCS):
        proc = mp.Process(target=experience_func, 
                          args=(policynet, targetvaluenet, train_queue))
        proc.start()
        procs.append(proc)
